...
export default function Products({}: Props) {
  const { categories, initializing, addToCart, cart, changeQty } = useContext(
    CartContext
  ) as CartContextType;
  ...
  const subtotal: number = cart.reduce((a, b) => a + b.price, 0);
  ...
  return (
    <div>
            ...
            {category.items?.map((product) => (
              <div
                className={classNames(
                  'col-span-6 md:col-span-3 xl:col-span-2 bg-white p-3 rounded-lg shadow-md border transition-all',
                  onCart(product.id!) ? 'border-teal-600' : 'border-transparent'
                )}
                key={product.id}
              >
                ...
                <div className="flex items-center justify-end mt-5">
                  {onCart(product.id!) ? (
                    ...
                    <button
                      ...
                      className="text-xs font-medium rounded-md shadow-md px-3 border border-red-600 text-red-600 hover:bg-red-50 h-8"
                    >
                      PESAN
                    </button>
                  )}
      ...
      {totalItems > 0 && (
        <div className="p-5 bg-white flex items-center sticky bottom-0 intro-y border-t border-red-500">
          ...
          <Button label="Keranjang Belanja" />
        </div>
      )}
    </div>
  );
